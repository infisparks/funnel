...
import { StandaloneSurveyClient } from './StandaloneSurveyClient';
...
async function fetchWorkspaceForSurvey(subdomain?: string, domain?: string, funnel_id?: string) {
  try {
    ...
    console.error('Error fetching workspace for survey:', err);
  }
  return null;
}
...
export default async function SurveyPage({
  ...
  const workspace = await fetchWorkspaceForSurvey(subdomainName, resolvedParams.domain, resolvedParams.funnel_id);

  return (
    <Suspense fallback={<div className="p-8 text-center text-amber-500 font-bold bg-[#0B0F17] min-h-screen">Loading Qualification Survey...</div>}>
      <StandaloneSurveyClient workspace={workspace} />
    </Suspense>
  );
}
