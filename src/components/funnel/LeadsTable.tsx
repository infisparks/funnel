...
export function LeadsTable() {
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    const isMeeting = lead.step_progress === 'meeting_booked';
    const hasSurvey = lead.step_progress === 'survey_completed' || Boolean(lead.survey_responses && Object.keys(lead.survey_responses).length > 0);

    // Status Filter
    const matchesStatus =
      statusFilter === 'all' ||
      (statusFilter === 'meeting_booked' && isMeeting) ||
      (statusFilter === 'survey_completed' && hasSurvey && !isMeeting) ||
      (statusFilter === 'step1_contact' && !hasSurvey && !isMeeting) ||
      lead.step_progress === statusFilter;
                  ...
                  return (
                    <tr key={lead.id} className="hover:bg-[#F9FAFB] transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        ) : lead.step_progress === 'survey_completed' || (lead.survey_responses && Object.keys(lead.survey_responses).length > 0) ? (
